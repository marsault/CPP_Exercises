...
1. Dans un nouveau fichier `src/image.hpp`, vous allez définir une classe template `Image<typename P, size_t W, size_t H>` où `W` est la largeur de l'image, `H` sa hauteur et `P` le type utilisé pour les pixels (au début, on utilisera le type `uint8_t`).
