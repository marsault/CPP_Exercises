...
Tout au long du TP, votre code produira des images dans le dossier `images` situé dans votre dossier de build (pas celui à la racine de votre dossier).

### A. Image

1. Dans un nouveau fichier `src/image.hpp`, vous allez définir une classe template `Image<typename P, size_t W, size_t H>` où `(W, H)` sont les dimensions de l'image et `P` le type utilisé pour les pixels (au début, on utilisera le type `uint8_t`).
...
2. Définissez les constructeurs suivants :
    ...
    - un constructeur par défault `Image()` (on n'a pas besoin d'initialiser les pixels, vous pouvez utiliser `default`).

3. Etant donné une `Image<..>` stockée dans une variable `img`, on veut aussi pouvoir utiliser `img(i,j)` pour accéder au pixel de la j-ème ligne et de la i-ème colonne.
...
4. Faites le nécessaire pour que l'on puisse additionner deux images de même tailles. Le résultat produira une image dont chaque pixel est la somme des deux pixels situés à la même position dans les deux images d'origine `img1` et `īmg2`.
...
6. On pourra tester le code avec le programme `grayscale.cpp`.  N'oubliez pas d'inclure `lib/image_lib.cpp` dans la compilation.
...
N'oubliez pas d'inclure `lib/image_lib.cpp` !
...
On veut pouvoir additioner les différents pixels qu'on a définis pour pouvoir combiner des images. On peut :
...
Appliquer le masque de transparence B à une image A voudra dire pour nous "remplacer le canal alpha de A par B" (B sera donc une image à un seul canal). Voici un exemple parlant:
