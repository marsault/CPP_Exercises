...
L'objectif est que les arguments du constructeur de l'objet soient passés à la fonction `register_entity`, puis que ces arguments soient "stockés" dans le builder.
   ...
   2. Dupliquer la fonction-template `register_entity` de manière à utiliser un deuxième paramètre template pour l'argument passé au constructeur.  
   Verifiez que:
      - la sortie pour l'entrée `Person`, la sortie soit bien `Person called Jean`;
      - que les autres sorties n'ont pas changé.

   3. Ajoutez maintenant ce qu'il faut pour que:
      - l'identifiant `Dog` produise un `Animal` dont l'espèce est `dog`
      - l'identifiant `Cat` produise un `Animal` dont l'espèce est `cat`
...
5. Gestion de n'importe quel nombre d'arguments du constructeur.
   ...
   2. Remplacez les fonction-template `register_entity` par une seule fonction qui utilise des paramètres de template variadiques pour faire compiler l'instruction précédente.  
