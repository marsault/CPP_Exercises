...
public:
    ...
    void can_eat_meat() const override { std::cout << "I'd rather eat grain'!" << std::endl; }
private:
...
{
    Chicken chicken;
    chicken.type();          // 1a  Chicken::type() -> linké statiquement
    chicken.can_eat_plant(); // 1b  Bird::can_eat_plant() -> il n'y a pas de fonction dans Chicken, on remonte à la classé hérité

    Herbivore& chicken_as_herb = chicken;
    chicken_as_herb.type();          // 2a  Ne compile pas -> Herbivore n'a pas de fonction type()
    chicken_as_herb.can_eat_meat();  // 2b  Chicken::can_eat_meat() -> on remonte à Herbivore::can_eat_meat qui est linké dynamiquement
    chicken_as_herb.can_eat_plant(); // 2c  Herbivore::can_eat_plant() car il manque un const dans Bird

    Animal& chicken_as_anim = chicken;
    chicken_as_anim.move(); // 3a  Chicken::move() -> linké dynamiquement malgré l'absence du mot-clef override
    chicken_as_anim.type(); // 3b  Animal::type() -> linké statiquement

    Bird& chicken_as_bird = chicken;
    chicken_as_bird.move();          // 4a  Chicken::move() -> comme 3a
    chicken_as_bird.can_eat_meat();  // 4b  Chicken::can_eat_meat() -> comme 2b
    chicken_as_bird.can_eat_plant(); // 4c  Bird::can_eat_plant() -> linké statiquement
