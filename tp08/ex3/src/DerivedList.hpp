...
public:
  DerivedList()
      : Base(DerivedList::_type)
  {}

  // Puisqu'on stocke un vecteur de unique_ptr; DerivedList n'est plus copiable
  // donc on réécrit la constructeur de copie !
  ...
  // Pour pouvoir ajouter un Base, on doit rajouter une fonction virtuelle
  // (pure) Base::new_copy() qui alloue une copie sur le tas.
  ...
  // Pour pouvoir ajouter une R-value de type Base, on doit rajouter une
  // fonction virtuelle (pure) Base::new_move() qui alloue une copie sur le tas.
...
private:
  // La classe Base n'est pas instanciable, alors pour la stocker je dois
  // utiliser une référence ou un pointeur.
  //
